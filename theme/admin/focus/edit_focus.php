...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
        ...
        <td><input type="text" name="photo" id="photo" value="<?php echo set_value('photo',$row['photo']);?>"/><?php echo form_error('photo');?> <a class="btn btn-primary" id="image1">上传图片</a></td>
    </tr>
    <tr>
        <td class="tableleft" width="15%">手机图片</td>
        <td><input type="text" name="m_photo" id="photo2" value="<?php echo set_value('m_photo',$row['m_photo']);?>"/><?php echo form_error('m_photo');?> <a class="btn btn-primary" id="image2">上传图片</a></td>
    </tr>
    <tr>
        <td class="tableleft" width="15%">URL</td>
        <td><input type="text" name="url" value="<?php echo set_value('url',$row['url']);?>"/><?php echo form_error('url');?></td>
    </tr>
    <tr>
        <td class="tableleft" width="15%">手机URL</td>
        <td><input type="text" name="m_url" value="<?php echo set_value('m_url',$row['m_url']);?>"/><?php echo form_error('m_url');?></td>
    </tr>
    <tr>
        <td class="tableleft" width="15%">是否手机显示</td>
        <td>
    <input type="radio" name="is_phone" value="1" <?php echo set_radio('is_phone',1,$row['is_phone']==1?true:false);?>/>显示
        <input type="radio" name="is_phone" value="2" <?php echo set_radio('is_phone',2,$row['is_phone']==2?true:false);?>/>不显示
        <?php echo form_error('is_phone');?>
        </td></tr>
    <tr>
